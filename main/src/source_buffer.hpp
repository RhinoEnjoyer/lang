...
struct src_buffer_t {
  ...
  static auto make(llvm::vfs::FileSystem* const fs,
                   const std::string& filepath) -> llvm::ErrorOr<src_buffer_t> {
  ...
  auto ref(const source_index_t index,
           const std::size_t length) -> const llvm::StringRef {
